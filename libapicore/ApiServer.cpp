...
Json::Value ApiConnection::getMinerStat1()
{
	...
	ostringstream totalMhMit; 
	ostringstream totalMhDcr; 
	ostringstream detailedMhMit;
	ostringstream detailedMhDcr;
	ostringstream tempAndFans;
	ostringstream poolAddresses;
	ostringstream invalidStats;
	
	totalMhMit << std::fixed << std::setprecision(0) << (p.rate() / 1000.0f) << ";" << s.getAccepts() << ";" << s.getRejects();
	...
	{
		detailedMhMit << std::fixed << std::setprecision(0) << (p.minerRate(i) / 1000.0f) << (((numGpus -1) > gpuIndex) ? ";" : "");
	...
	jRes[0] = mitminer_get_buildinfo()->project_name_with_version;  //miner version.
	jRes[1] = toString(runningTime.count()); // running time, in minutes.
	jRes[2] = totalMhMit.str();              // total MIT hashrate in MH/s, number of MIT shares, number of MIT rejected shares.
	jRes[3] = detailedMhMit.str();           // detailed hashrate for all GPUs.
	...
	jRes[8] = invalidStats.str();            // number of MIT invalid shares, number of MIT pool switches, number of DCR invalid shares, number of DCR pool switches.
...
Json::Value ApiConnection::getMinerStatHR()
{
	...
	version << mitminer_get_buildinfo()->project_name_with_version;
	...
	for (unsigned gpuIndex = 0; gpuIndex < p.minersHashes.size(); gpuIndex++)
	{
		...
		detailedMhMit[gpuIndex] = (p.minerRate(minerhashes));
	...
	// total MIT hashrate in MH/s, number of MIT shares, number of MIT rejected shares.
	jRes["mithashrate"] = (p.rate());
	jRes["mithashrates"] = detailedMhMit;
	jRes["mitshares"] 	= s.getAccepts();
	jRes["mitrejected"] = s.getRejects();
	jRes["mitinvalid"] 	= s.getFailures();
	jRes["mitpoolsw"] 	= 0;
	// Hardware Info
