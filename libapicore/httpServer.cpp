...
void httpServer::tableHeader(stringstream& ss, unsigned columns)
{
    ...
    ss <<
       ...
       mitminer_get_buildinfo()->project_name_with_version <<
       " on " << hostName << " - " << l << "</th></tr>";
}
...
void httpServer::run(string address, uint16_t port, dev::mit::Farm* farm, bool show_hwmonitors, bool show_power)
{
	if (port == 0)
		return;
    m_farm = farm;
