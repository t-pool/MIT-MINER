...
const char* LogChannel::name() { return MitGray ".."; }
const char* WarnChannel::name() { return MitRed " X"; }
const char* NoteChannel::name() { return MitBlue " i"; }
	...
	m_verbosity(_v)
{
	...
	if ((int)_v <= g_logVerbosity)
	{
		...
		if (g_logSyslog)
			m_sstr << std::left << std::setw(8) << getThreadName() << " " MitReset;
		else {
			time_t rawTime = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
			char buf[24];
			if (strftime(buf, 24, "%X", localtime(&rawTime)) == 0)
				buf[0] = '\0'; // empty if case strftime fails
			m_sstr << _id << " " MitViolet << buf << " " MitNavy << std::left << std::setw(8) << getThreadName() << " " MitReset;
		}
	}
}
