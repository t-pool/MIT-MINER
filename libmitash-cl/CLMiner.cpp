...
{
    static const char* name() { return MitOrange "cl"; }
    static const int verbosity = 2;
    static const bool debug = false;
};
#define cllog clog(CLChannel)
#define MITCL_LOG(_contents) cllog << _contents
 ...
 */
static std::string mitCLErrorHelper(const char *msg, cl::Error const &clerr) {
...
void CLMiner::workLoop()
{
        ...
        while (!shouldStop())
        {
            ...
            for (uint32_t i = 0; i < results.count; i++) {
                ...
                if (nonce != m_lastNonce)
                {
                    ...
                    {
                        Result r = MitashAux::eval(current.epoch, current.header, nonce);
    ...
    {
        cwarn << MitCLErrorHelper("OpenCL Error", _e);
        if(s_exit)
            exit(1);
    }
}
    ...
    bool _exit, bool _nobinary)
{
    ...
    auto dagSize = mitash::get_full_dataset_size(mitash::calculate_full_dataset_num_items(epoch));
...
bool CLMiner::init(int epoch)
{
    ...
    try
    {
        ...
        MITCL_LOG("Platform: " << platformName);
        ...
        if (devices.empty())
        {
            MITCL_LOG("No OpenCL devices found.");
            return false;
        }
        ...
        MITCL_LOG("Device:   " << device_name << " / " << device_version);
        ...
        if (clVer == "1.0" || clVer == "1.1")
        {
            ...
            {
                MITCL_LOG("OpenCL " << clVer << " not supported, but platform Clover might work nevertheless. USE AT OWN RISK!");
            }
            else
            {
                MITCL_LOG("OpenCL " << clVer << " not supported - minimum required version is 1.2");
                return false;
            }
        }
        ...
        const auto& context = mitash::get_global_epoch_context(epoch);
        const auto lightNumItems = context.light_cache_num_items;
        const auto lightSize = mitash::get_light_cache_size(lightNumItems);
        m_dagItems = context.full_dataset_num_items;
        const auto dagSize = mitash::get_full_dataset_size(m_dagItems);
        ...
        cllog << "OpenCL kernel";
        code = string(mitash_cl, mitash_cl + sizeof(mitash_cl));
        ...
        if (!s_noBinary) {
            ...
            /* Open kernels/mitash_{devicename}_lws{local_work_size}.bin */
        ...
        {
            cwarn << mitCLErrorHelper("Creating DAG buffer failed", err);
            return false;
        }
        // create buffer for header
        MITCL_LOG("Creating buffer for header.");
        m_header.clear();
        ...
        MITCL_LOG("Creating mining buffer");
    ...
    {
        cwarn << mitCLErrorHelper("OpenCL init failed", err);
        if(s_exit)
            exit(1);
        return false;
    }
