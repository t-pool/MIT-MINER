...
{
    static const char* name() { return MitOrange "cu"; }
    ...
    )
{
    try
    {
        ...
        const auto dagSize =
            mitash::get_full_dataset_size(mitash::calculate_full_dataset_num_items(0));
    ...
    unsigned dagCreateDevice)
{
    try
    {
        ...
        const auto dagSize = mitash::get_full_dataset_size(dagNumItems);
        ...
        set_constants(dag, dagNumItems, light, lightNumItems); //in mitash_cuda_miner_kernel.cu
        ...
        if (dagNumItems != m_dag_size || !dag)
        {
            ...
            if (!hostDAG)
            {
                if((m_device_num == dagCreateDevice) || !_cpyToHost){ //if !cpyToHost -> All devices shall generate their DAG
                    ...
                    mitash_generate_dag(dagSize, s_gridSize, s_blockSize, m_streams[0]);
    ...
    const dev::mit::WorkPackage& w)
{
         ...
         current_index++, current_nonce += batch_size)
    {
        ...
        run_mitash_search(s_gridSize, s_blockSize, stream, buffer, current_nonce, m_parallelHash);
    }
        ...
        for (current_index = 0; current_index < s_numStreams; current_index++, current_nonce += batch_size) {
            ...
            if (found_count) {
                ...
                for (uint32_t i = 0; i < found_count; i++)
                    ...
                    {
                        Result r = MitashAux::eval(w.epoch, w.header, nonces[i]);
