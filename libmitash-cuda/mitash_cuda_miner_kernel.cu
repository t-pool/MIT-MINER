/*
* Genoil's CUDA mining kernel for mitereum
* based on Tim Hughes' opencl kernel.
* thanks to sp_, trpuvot, djm34, cbuchner for things i took from ccminer.
*/
#include "mitash_cuda_miner_kernel.h"
#include "mitash_cuda_miner_kernel_globals.h"
...
)
{
	switch (parallelHash)
	{
		case 1: mitash_search <1> <<<gridSize, blockSize, 0, stream >>>(g_output, start_nonce); break;
		case 2: mitash_search <2> <<<gridSize, blockSize, 0, stream >>>(g_output, start_nonce); break;
		case 4: mitash_search <4> <<<gridSize, blockSize, 0, stream >>>(g_output, start_nonce); break;
		case 8: mitash_search <8> <<<gridSize, blockSize, 0, stream >>>(g_output, start_nonce); break;
		default: mitash_search <4> <<<gridSize, blockSize, 0, stream >>>(g_output, start_nonce); break;
	}
	CUDA_SAFE_CALL(cudaGetLastError());
}

#define mitASH_DATASET_PARENTS 256
#define NODE_WORDS (64/4)


__global__ void
mitash_calculate_dag_item(uint32_t start)
{
	...
	for (uint32_t i = 0; i != mitASH_DATASET_PARENTS; ++i) {
	...
	)
{
	...
	{
		mitash_calculate_dag_item <<<gridSize, blockSize, 0, stream>>>(base);
	...
	if (base < work)
	{
		...
		mitash_calculate_dag_item <<<lastGrid, blockSize, 0, stream>>>(base);
		CUDA_SAFE_CALL(cudaDeviceSynchronize());
	}
	CUDA_SAFE_CALL(cudaGetLastError());
}
