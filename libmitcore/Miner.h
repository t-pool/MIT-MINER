...
inline std::ostream& operator<<(std::ostream& _out, WorkingProgress _p)
{
	...
	_out << "Speed "
		 << MitTealBold << std::fixed << std::setprecision(2) << mh << MitReset
		 << " Mh/s";
	...
	for (size_t i = 0; i < _p.minersHashes.size(); ++i)
	{
		...
		_out << " gpu" << i << " " << MitTeal << std::fixed << std::setprecision(2) << mh << MitReset;
		if (_p.minerMonitors.size() == _p.minersHashes.size())
			_out << " " << MitTeal << _p.minerMonitors[i] << MitReset;
	}
