...
MitGetworkClient::MitGetworkClient(unsigned farmRecheckPeriod, bool submitHashrate)
...
MitGetworkClient::~MitGetworkClient()
{
    p_client = nullptr;
}

void MitGetworkClient::connect()
{
...
void MitGetworkClient::disconnect()
{
...
void MitGetworkClient::submitHashrate(string const& rate)
{
...
void MitGetworkClient::submitSolution(const Solution& solution)
{
    ...
    if (m_connected.load(std::memory_order_relaxed))
    {
        try
        {
            bool accepted = p_client->mit_submitWork("0x" + toHex(solution.nonce),
...
void MitGetworkClient::workLoop()
{
    while (!shouldStop())
    {
        if (m_connected.load(std::memory_order_relaxed))
        {
            ...
            try
            {
                ...
                newWorkPackage.epoch = mitash::find_epoch_number(
                    mitash::hash256_from_bytes(h256{v[1].asString()}.data()));
