#include "zmitStratumClient.h"
#include <libdevcore/Log.h>

#include <mitminer/buildinfo.h>
...
MitStratumClient::MitStratumClient(boost::asio::io_service & io_service, int worktimeout, int responsetimeout, const string& email, bool submitHashrate) : PoolClient(),
...
MitStratumClient::~MitStratumClient()
{
    // Do not stop io service.
    // It's global
}

void MitStratumClient::connect()
{
    ...
    m_resolver.async_resolve(q,
        m_io_strand.wrap(boost::bind(&MitStratumClient::resolve_handler, this, boost::asio::placeholders::error, boost::asio::placeholders::iterator)));
    
}

void MitStratumClient::disconnect()
{
    ...
    if (m_socket && m_socket->is_open()) { 
        ...
        try {
            ...
            if (m_conn->SecLevel() != SecureLevel::NONE) {
                ...
                m_securesocket->async_shutdown(m_io_strand.wrap(boost::bind(&MitStratumClient::onSSLShutdownCompleted, this, boost::asio::placeholders::error)));

                m_conntimer.expires_from_now(boost::posix_time::seconds(m_responsetimeout));
                m_conntimer.async_wait(boost::bind(&MitStratumClient::check_connect_timeout, this, boost::asio::placeholders::error));
...
void MitStratumClient::disconnect_finalize() {
    ...
    if (!m_conn->StratumModeConfirmed() && m_canconnect.load(std::memory_order_relaxed))
    {
        ...
        if (m_conn->StratumMode() > 0)
        {
            ...
            m_io_service.post(m_io_strand.wrap(boost::bind(&MitStratumClient::connect, this)));
            return;
        }
        else
        {
...
void MitStratumClient::resolve_handler(const boost::system::error_code& ec, tcp::resolver::iterator i)
{   
    if (!ec)
    {
        ...
        m_io_service.post(m_io_strand.wrap(boost::bind(&MitStratumClient::start_connect, this)));
...
void MitStratumClient::reset_work_timeout()
{
    m_worktimer.cancel();
    m_worktimer.expires_from_now(boost::posix_time::seconds(m_worktimeout));
    m_worktimer.async_wait(m_io_strand.wrap(boost::bind(&MitStratumClient::work_timeout_handler, this, boost::asio::placeholders::error)));

}

void MitStratumClient::start_connect()
{
    if (!m_endpoints.empty()) {
        ...
        m_conntimer.expires_from_now(boost::posix_time::seconds(m_responsetimeout));
        m_conntimer.async_wait(m_io_strand.wrap(boost::bind(&MitStratumClient::check_connect_timeout, this, boost::asio::placeholders::error)));
        
        // Start connecting async
        if (m_conn->SecLevel() != SecureLevel::NONE) {
            m_securesocket->lowest_layer().async_connect(m_endpoint, m_io_strand.wrap(boost::bind(&MitStratumClient::connect_handler, this, _1)));
        }
        else {
            m_socket->async_connect(m_endpoint,
                m_io_strand.wrap(boost::bind(&MitStratumClient::connect_handler, this, _1)));
        }
...
void MitStratumClient::check_connect_timeout(const boost::system::error_code& ec)
{
    (void)ec;
...
void MitStratumClient::connect_handler(const boost::system::error_code& ec)
{
    ...
    if (!m_socket->is_open()) {
        ...
        - 1st pass MitStratumClient::MitEREUMSTRATUM  (2)
        - 2nd pass MitStratumClient::MitPROXY         (1)
        - 3rd pass MitStratumClient::STRATUM          (0)
        */
        ...
        if (!m_conn->StratumModeConfirmed())
        {
            ...
            case 2:
                ...
                jReq["params"].append(mitminer_get_buildinfo()->project_name_with_version);
                jReq["params"].append("MitereumStratum/1.0.0");
                break;
        ...
        else
        {
            switch (m_conn->StratumMode())
            {
            case MitStratumClient::STRATUM:

                jReq["jsonrpc"] = "2.0";

                break;

            case MitStratumClient::MITPROXY:
            ...
            case MitStratumClient::MITEREUMSTRATUM:
        ...
        */
        ...
        m_responsetimer.async_wait(m_io_strand.wrap(boost::bind(&MitStratumClient::response_timeout_handler, this, boost::asio::placeholders::error)));
        sendSocketData(jReq);
...
std::string MitStratumClient::processError(Json::Value& responseObject)
{
    std::string retVar;
...
void MitStratumClient::processExtranonce(std::string& enonce)
{
    m_extraNonceHexSize = enonce.length();

    cnote << "Extranonce set to " MitWhite << enonce << MitReset " (nicehash)";
    enonce.append(16 - m_extraNonceHexSize, '0');
    m_extraNonce = h64(enonce);
}

void MitStratumClient::processResponse(Json::Value &responseObject)
{
    ...
    if (_isNotification && _method == "" && m_conn->StratumMode() == MitStratumClient::MITPROXY && responseObject["result"].isArray()) {
    ...
    { 
        ...
        cwarn << "Do not blame Mitminer for this. Ask pool devs to honor http://www.jsonrpc.org/ specifications ";
        cwarn << "Disconnecting ...";
        m_io_service.post(m_io_strand.wrap(boost::bind(&MitStratumClient::disconnect, this)));
        return;
    ...
    if (!_isNotification) {
        ...
        case 1:
            ...
            */
            if (!m_conn->StratumModeConfirmed())
            {
                switch (m_conn->StratumMode())
                {
                case MitStratumClient::ETHEREUMSTRATUM:

                    // In case of success we also need to verify third parameter of "result" array
                    // member is exactly "MitereumStratum/1.0.0". Otherwise try with another mode
                    if (_isSuccess &&
                        jResult.isArray() && jResult[0].isArray() && jResult[0].size() == 3 &&
                        jResult[0].get((Json::Value::ArrayIndex)2, "").asString() == "MitereumStratum/1.0.0")
                    {
                        // ETHEREUMSTRATUM is confirmed
                        cnote << "Stratum mode detected : MITEREUMSTRATUM (NiceHash)";
                ...
                case MitStratumClient::MITPROXY:
                    ...
                    else
                    {
                        ...
                        cnote << "Stratum mode detected : MITPROXY compatible";
            ...
            // Result should be an array with multiple dimensions, we only care about the data if MitStratumClient::ETHEREUMSTRATUM
            switch (m_conn->StratumMode()) {
            ...
            case MitStratumClient::ETHPROXY:
                ...
                if (!m_subscribed)
                {
                    ...
                    cnote << "Logged in to mit-proxy server";
                    m_authorized.store(true, std::memory_order_relaxed);

                    jReq["id"] = unsigned(5);
                    jReq["method"] = "mit_getWork";
                    jReq["params"] = Json::Value(Json::arrayValue);
        ...
        case 5:
            ...
            if (m_conn->StratumMode() == MitStratumClient::MITPROXY && responseObject["result"].isArray()) {
                _method = "mining.notify";
    ...
    if (_isNotification && m_conn->StratumModeConfirmed()) {
        ...
        if (_method == "mining.notify")
        {
            ...
            */
            ...
            if (m_conn->StratumMode() == MitStratumClient::MITPROXY && responseObject.isMember("result")) {
            ...
            if (jPrm.isArray() && !jPrm.empty())
            {
                ...
                if (m_conn->StratumMode() == MitStratumClient::MITEREUMSTRATUM)
                {
                    ...
                    if (sHeaderHash != "" && sSeedHash != "")
                    {
                        ...
                        m_current.epoch = mitash::find_epoch_number(mitash::hash256_from_bytes(h256{sSeedHash}.data()));
                ...
                else
                {
                    ...
                    if (sHeaderHash != "" && sSeedHash != "" && sShareTarget != "")
                    {
                        ...
                        if (headerHash != m_current.header)
                        {
                            ...
                            m_current.header = h256(sHeaderHash);
                            m_current.epoch = Mitash::find_epoch_number(
                                Mitash::hash256_from_bytes(h256{sSeedHash}.data()));
        ...
        else if (_method == "mining.set_difficulty" && m_conn->StratumMode() == MitStratumClient::MITEREUMSTRATUM)
        {
            jPrm = responseObject.get("params", Json::Value::null);
            if (jPrm.isArray())
            {
                ...
                cnote << "Difficulty set to " MitWhite << nextWorkDifficulty << MitReset " (nicehash)";
                if (m_onWorkReceived && (m_current.epoch != -1)) {
                    m_onWorkReceived(m_current, false);
                }
            }
        }
        else if (_method == "mining.set_extranonce" && m_conn->StratumMode() == MitStratumClient::MITEREUMSTRATUM)
        {
            jPrm = responseObject.get("params", Json::Value::null);
            if (jPrm.isArray())
            {
        ...
        else if (_method == "client.get_version")
        {
            ...
            jReq["result"] = Mitminer_get_buildinfo()->project_name_with_version;
...
void MitStratumClient::work_timeout_handler(const boost::system::error_code& ec) {
    if (!ec)
    {
        if (isConnected())
        {
...
void MitStratumClient::response_timeout_handler(const boost::system::error_code& ec) {
    if (!ec)
    {
        if (isConnected())
        {
            ...
            if (m_conn->StratumModeConfirmed() == false && m_conn->IsUnrecoverable() == false)
            {
                ...
                jRes["error"] = true;
                m_io_service.post(
                    m_io_strand.wrap(boost::bind(&MitStratumClient::processResponse, this, jRes)));
            }
        }
    }
}

void MitStratumClient::submitHashrate(string const & rate) {
    ...
    // There is no stratum mMitod to submit the hashrate so we use the rpc variant.
    // Note !!
    // id = 6 is also the id used by Mitermine.org and nanopool to push new jobs
    ...
    jReq["method"] = "mit_submitHashrate";
    jReq["params"] = Json::Value(Json::arrayValue);
...
void MitStratumClient::submitSolution(const Solution& solution) {
    ...
    m_responsetimer.async_wait(m_io_strand.wrap(boost::bind(
        &MitStratumClient::response_timeout_handler, this, boost::asio::placeholders::error)));
...
void MitStratumClient::recvSocketData() {
    if (m_conn->SecLevel() != SecureLevel::NONE)
    {
        async_read_until(*m_securesocket, m_recvBuffer, "\n",
            m_io_strand.wrap(boost::bind(&MitStratumClient::onRecvSocketDataCompleted, this,
...
void MitStratumClient::onRecvSocketDataCompleted(const boost::system::error_code& ec, std::size_t bytes_transferred) {
    ...
    if (!ec && bytes_transferred > 0) {
        ...
        if (isConnected())
        {
            if (!message.empty())
            {
                ...
                {
                    m_io_service.post(boost::bind(&MitStratumClient::processResponse, this, jMsg));
                }
                else
                {
...
void MitStratumClient::sendSocketData(Json::Value const & jReq) {
    if (!isConnected())
        return;
    ...
    if (m_conn->SecLevel() != SecureLevel::NONE)
    {
        async_write(*m_securesocket, m_sendBuffer,
            m_io_strand.wrap(boost::bind(&MitStratumClient::onSendSocketDataCompleted, this,
...
void MitStratumClient::onSendSocketDataCompleted(const boost::system::error_code& ec) {
    if (ec)
    {
        if ((ec.category() == boost::asio::error::get_ssl_category()) &&
            (SSL_R_PROTOCOL_IS_SHUTDOWN == ERR_GET_REASON(ec.value())))
        {
            ...
            m_io_strand.wrap(boost::bind(&MitStratumClient::disconnect, this));
        }
...
void MitStratumClient::onSSLShutdownCompleted(const boost::system::error_code& ec) {
    (void)ec;
    // cnote << "onSSLShutdownCompleted Error code is : " << ec.message();
    m_io_service.post(m_io_strand.wrap(boost::bind(&MitStratumClient::disconnect_finalize, this)));
}
