...
#include <libmitcore/MitashAux.h>
...
class MitStratumClient : public PoolClient
{
public:

	typedef enum { STRATUM = 0, MITPROXY, MITEREUMSTRATUM } StratumProtocol;

	MitStratumClient(boost::asio::io_service & io_service, int worktimeout, int responsetimeout, const string& email, bool submitHashrate);
	~MitStratumClient();
...
private:
    ...
    string m_worker;  // Mit-proxy only; No ! It's for all !!!
