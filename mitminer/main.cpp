...
#if MIT_MITASHCL
#include <libmitash-cl/CLMiner.h>
#endif
#if MIT_MITASHCUDA
#include <libmitash-cuda/CUDAMiner.h>
#endif
#include <libpoolprotocols/PoolManager.h>
#include <libpoolprotocols/stratum/MitStratumClient.h>
#include <libpoolprotocols/getwork/MitGetworkClient.h>
...
using namespace std;
using namespace dev;
using namespace dev::mit;

struct MiningChannel: public LogChannel
{
	static const char* name() { return MitGreen " m"; }
...
public:
	...
	void io_work_timer_handler(const boost::system::error_code& ec) {
		...
		if (!ec) {
			...
			// ensuring io_service's queue has always somMiting to do
	...
	void ParseCommandLine (int argc, char** argv)
	{
		...
		CLI::App app("Mitminer - GPU Mitereum miner");
...
#if MIT_MITASHCL || MIT_MITASHCUDA
    	...
    	try {
			...
			if (help) {
			    ...
			    auto* bi = mitminer_get_buildinfo();
    			cerr << "\nmitminer " << bi->project_version << "\nBuild: " << bi->system_name << "/"
	...
	void execute()
	{
		...
		auto* build = mitminer_get_buildinfo();
		minelog << "mitminer " << build->project_version;
...
#else
			cerr << endl << "Selected GPU mining without having compiled with -DMITASHCL=ON" << "\n\n";
			exit(1);
#endif
		}
		if (m_minerType == MinerType::CUDA || m_minerType == MinerType::Mixed)
...
#else
			cerr << endl << "CUDA support disabled. Configure project build with -DMITASHCUDA=ON" << "\n\n";
			stop_io_service();
			exit(1);
#endif
		}
...
private:
	...
	void doMiner()
	{
		...
		if (m_mode == OperationMode::Stratum) {
			client = new MitStratumClient(m_io_service, m_worktimeout, m_responsetimeout, m_email, m_report_hashrate);
		}
		else if (m_mode == OperationMode::Farm) {
			client = new MitGetworkClient(m_farmRecheckPeriod, m_report_hashrate);
		}
